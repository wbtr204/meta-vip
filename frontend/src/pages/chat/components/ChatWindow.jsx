...
const ChatWindow = ({
	...
	return (
		<div className="relative h-full w-full bg-white dark:bg-slate-950 overflow-hidden">
			{/* Header - Pin to Top */}
			<div className="absolute top-0 left-0 right-0 z-30 border-b border-slate-200 bg-white px-4 py-4 dark:border-slate-800 dark:bg-slate-950 sm:px-6 h-[72px]">
				<div className="flex items-center justify-between gap-3 h-full">
					<div className="flex min-w-0 items-center gap-3">
						...
						<div className="relative shrink-0">
							<div className="h-10 w-10 overflow-hidden rounded-2xl border border-slate-200 shadow-sm dark:border-slate-800">
								<img
						...
						<div className="min-w-0">
							<div className="truncate text-[14px] font-bold tracking-tight text-slate-900 dark:text-white">
								{selectedConversation.user.fullName}
							</div>
							<div className="flex items-center gap-1.5">
								<span className={`h-1.5 w-1.5 rounded-full ${isPending ? "bg-slate-400" : "bg-emerald-500"}`} />
								<p className="truncate text-[9px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400">
									{isPending ? "Chờ duyệt" : "Online"} · @{selectedConversation.user.username}
								</p>
							</div>
						</div>
					</div>
					...
					<div className="flex items-center gap-1">
						{!isPending && (
							<>
								<button className="rounded-xl p-2 text-slate-500 transition-colors hover:bg-slate-100 hover:text-indigo-500 dark:hover:bg-slate-800">
									<Phone size={16} />
								</button>
								<button className="rounded-xl p-2 text-slate-500 transition-colors hover:bg-slate-100 hover:text-indigo-500 dark:hover:bg-slate-800">
									<Video size={16} />
						...
						<button className="rounded-xl p-2 text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-900 dark:hover:bg-slate-800 dark:hover:text-white">
							<Info size={16} />
						</button>
					</div>
				</div>
			...
			{/* Message List - Dynamic space between Header and Input */}
			<div 
				ref={scrollRef} 
				className="absolute top-[72px] bottom-[72px] left-0 right-0 overflow-y-auto custom-scrollbar bg-slate-50/40 dark:bg-slate-950/30 pb-4 px-4 py-4 sm:px-6"
			>
				{isLoading ? (
							...
							<div className="mb-4 rounded-2xl border border-amber-500/20 bg-amber-500/10 p-4">
								<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
									<div className="flex items-start gap-3">
										...
										<div className="min-w-0">
											<p className="text-sm font-bold text-amber-800 dark:text-amber-200 leading-none">
												Yêu cầu nhắn tin
											</p>
									...
									<div className="flex items-center gap-2">
										<button
											...
											className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 rounded-xl bg-slate-900 px-4 py-2 text-xs font-bold text-white transition-all disabled:opacity-50 dark:bg-white dark:text-slate-900"
										>
											{isAccepting ? <LoadingSpinner size="xs" /> : "Chấp nhận"}
										</button>
										<button
											onClick={() => rejectRequest()}
											disabled={isAccepting || isRejecting}
											className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-xs font-bold text-slate-700 disabled:opacity-50 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300"
										>
											{isRejecting ? <LoadingSpinner size="xs" /> : "Xóa"}
										</button>
									</div>
								</div>
							</div>
						...
						<AnimatePresence>
							{isTyping && (
								<motion.div
									...
									exit={{ opacity: 0, x: -10 }}
									className="mt-4 ml-10 flex items-center gap-2"
								>
						...
						{isPending && isInitiator && (
							<div className="mt-4 rounded-2xl border border-indigo-500/20 bg-indigo-500/10 p-4">
								<p className="text-xs font-bold text-indigo-700 dark:text-indigo-300">
									Yêu cầu đã gửi. Chờ đối phương chấp nhận.
								</p>
							</div>
						)}
					</>
			...
			{/* Input Bar - Pin to Bottom */}
			<div className="absolute bottom-0 left-0 right-0 z-30 h-[72px]">
				<ChatInput
					onSend={onSend}
					onTyping={onTyping}
					isPending={isPending}
